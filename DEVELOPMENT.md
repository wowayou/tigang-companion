...
- **已知限制**:① PWA 不后台同步——只在打开应用时同步;② 同 dateStr 同内容(撞指纹)去重;③ **同一天多次不同内容的训练按「日」合并,LWW 只留 ts 最大的一条**(按日粒度的刻意简化;要按「次」保留需给记录加唯一 id,属设计变更);④ 主密码丢失=远端密文不可恢复(本地不丢);⑤ 限流 1 次/10s per userId + 20 次/分 per IP;⑥ 单机房凤凰城,机器挂=同步断(降级本地不丢);⑦ 迁移性:client.mjs 抽象了 origin,未来可换 Worker 后端,端到端加密不变。
