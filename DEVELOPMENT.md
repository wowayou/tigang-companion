...
- **相位环**:自定义属性默认不可补间,换阶段时 `--ring` 从 0% 硬跳回 100%。用 `@property` 注册 `--ring`/`--ring-color`/`--ring-track` 后才能过渡。`--ring` 的过渡必须短(.15s):它同时也在平滑每 100ms 一次的写入,太长会让环明显滞后于倒计时。不支持 `@property` 的浏览器退回硬切,功能不受影响。
